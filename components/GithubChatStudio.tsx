...
export const GithubChatStudio: React.FC<{
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full w-full bg-slate-100 overflow-hidden font-sans border-0 rounded-none relative">
      ...
      <div className="hidden lg:flex lg:w-[380px] xl:w-[420px] 2xl:w-[460px] flex-col bg-white border-r border-slate-200 shrink-0 h-full overflow-hidden">
        ...
        <div className="shrink-0 flex items-center justify-between px-4 py-2.5 border-b border-slate-200 bg-white gap-2">
          <div className="flex items-center gap-2 min-w-0">
            <button
              onClick={onBack}
              className="p-1 rounded-lg text-slate-500 hover:bg-slate-100 transition-colors hidden lg:block shrink-0 cursor-pointer"
              title="Back"
            >
            ...
            <button
              ...
              className="p-1 rounded-lg text-slate-500 hover:bg-slate-100 transition-colors lg:hidden shrink-0 cursor-pointer"
              title="Close"
            >
              <X className="w-4 h-4" />
            </button>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 space-y-4 bg-white text-sm">
          ...
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} min-w-0`}>
              <div
                className={`rounded-2xl text-sm sm:text-base leading-relaxed whitespace-pre-wrap [word-break:normal] [overflow-wrap:break-word] break-words ${m.role === 'user'
        ...
        <div className="shrink-0 p-3.5 bg-white border-t border-slate-200 flex flex-col gap-2">
          <div className="flex items-center gap-1.5 flex-wrap">
            <button
              onClick={() => send("Please enhance my tech stack with modern, relevant developer badges and tools, and ensure my GitHub stats and streak cards are enabled and highlighted.")}
              disabled={loading}
            ...
            <button
              onClick={() => send("Please optimize and polish my GitHub README: make my bio punchy and developer-focused, rewrite expertise bullet points with impactful details, and polish the layout.")}
              disabled={loading}
              className="px-3 py-1.5 text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-lg transition-colors flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed shadow-2xs cursor-pointer"
            >
              <Sparkles className="w-3.5 h-3.5 text-slate-500" />
              Optimize & Enhance README
            </button>
          </div>
      ...
      <div className="w-full lg:w-auto flex-1 min-w-0 h-full flex flex-col bg-slate-100/90 overflow-hidden relative">
        {/* Top Header of Preview Column */}
        <div className="shrink-0 flex items-center justify-between px-2 sm:px-4 py-2 border-b border-slate-200 bg-white shadow-2xs z-30 min-w-0 overflow-visible">
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 shrink-0 ml-auto">
            ...
            <button
              ...
              className="w-7 h-7 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 transition-colors flex items-center justify-center cursor-pointer shrink-0"
            >
            ...
            <button
              ...
              title={unlocked === false ? 'Unlock README.md' : 'Download README.md'}
              className="w-7 h-7 rounded-lg bg-slate-900 hover:bg-slate-800 text-white transition-colors flex items-center justify-center cursor-pointer shadow-xs shrink-0"
            >
        ...
        <div className="flex-1 min-w-0 overflow-y-auto p-4 sm:p-6 lg:p-8 flex justify-center items-start relative">
          <GithubReadmePreview
            github={github}
            editable={true}
            onSet={set}
      ...
      {showBannerPicker && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setShowBannerPicker(false);
          }}
        >
          <div className="w-full max-w-lg bg-white rounded-2xl shadow-2xl p-5 space-y-4 max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between">
              <span className="text-base font-bold text-slate-800">Choose a Banner Template</span>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-3 bg-slate-50 rounded-xl border border-slate-200">
              <div className="text-xs text-slate-600">
                <span className="font-semibold text-slate-800">Custom Banner:</span> Have your own image?
              </div>
              <button
                type="button"
                onClick={() => {
                ...
                className="flex items-center gap-2 px-3.5 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold cursor-pointer transition-colors shadow-2xs shrink-0"
              >
            ...
            <div className="grid grid-cols-2 gap-3">
              {BANNER_PRESETS.map((b) => (
                <button
                  ...
                  className={`rounded-xl border-2 overflow-hidden transition-all hover:scale-[1.01] cursor-pointer shadow-2xs ${
                    github.bannerUrl === b.url ? 'border-indigo-600 shadow-md ring-2 ring-indigo-500/20' : 'border-slate-200 hover:border-slate-300'
                  }`}
                >
                  <img src={b.thumb} alt={b.label} className="w-full h-14 object-cover" />
                  <span className="block text-xs font-semibold text-slate-700 py-1.5 text-center bg-white">{b.label}</span>
                </button>
              ))}
            </div>
            ...
            <div className="flex gap-2 pt-1 border-t border-slate-100">
              <input
                ...
                className="flex-1 text-xs px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-400 text-slate-700 placeholder-slate-400"
              />
              <button
                onClick={() => {
                ...
                className="px-4 py-2 rounded-lg bg-indigo-600 text-white text-xs font-bold disabled:opacity-40 hover:bg-indigo-500 transition-colors cursor-pointer"
              >
                Use
              </button>
            </div>
