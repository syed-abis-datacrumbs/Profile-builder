            ...
            <div className={`absolute bottom-full mb-2 ${isCollapsed ? 'left-12 w-56' : 'left-0 right-0 w-full'} bg-white rounded-2xl border border-slate-200 shadow-2xl p-3 space-y-3 z-50 animate-in fade-in slide-in-from-bottom-2 text-slate-900`}>
              ...
              <div className="pt-2 border-t border-slate-200 space-y-0.5 text-xs font-semibold text-slate-700">
                ...
                <div className="relative">
                  <button
                    type="button"
                    onClick={() => setIsTourSubmenuOpen((prev) => !prev)}
                    className="w-full flex items-center justify-between px-2.5 py-2 rounded-xl hover:bg-slate-100/80 text-slate-700 transition-colors text-left cursor-pointer"
                  >
                    <div className="flex items-center gap-3">
                      <HelpCircle className="w-4 h-4 shrink-0 text-indigo-600" />
                      <span>Product Tour</span>
                    </div>
                    <ChevronRight className={`w-3.5 h-3.5 text-slate-400 transition-transform ${isTourSubmenuOpen ? 'rotate-90' : ''}`} />
                  </button>

                  {isTourSubmenuOpen && (
                    <div className="ml-3 my-1 pl-2.5 border-l-2 border-indigo-200 space-y-1 py-1 animate-in fade-in slide-in-from-left-1">
                      <button
                        type="button"
                        onClick={() => {
                          setIsSettingsOpen(false);
                          ...
                          window.dispatchEvent(new CustomEvent('workspace_open_resume_studio_tour'));
                          setTimeout(() => {
                            if (onOpenTour) onOpenTour();
                          }, 150);
                        }}
                        className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-indigo-50 text-slate-800 text-xs font-semibold transition-colors text-left cursor-pointer"
                      >
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          toast.success('LinkedIn Studio Tour coming soon!');
                        }}
                        className="w-full flex items-center justify-between px-2 py-1.5 rounded-lg hover:bg-slate-100 text-slate-500 text-xs font-medium transition-colors text-left cursor-pointer"
                      >
                        <div className="flex items-center gap-2">
                          <Sparkles className="w-3.5 h-3.5 text-sky-500 shrink-0" />
                          <span>LinkedIn Tour</span>
                        </div>
                        <span className="text-[9px] font-bold text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded">Soon</span>
                      </button>

                      <button
                        type="button"
                        onClick={() => {
                          toast.success('GitHub Studio Tour coming soon!');
                        }}
                        className="w-full flex items-center justify-between px-2 py-1.5 rounded-lg hover:bg-slate-100 text-slate-500 text-xs font-medium transition-colors text-left cursor-pointer"
                      >
                        <div className="flex items-center gap-2">
                          <Code2 className="w-3.5 h-3.5 text-purple-500 shrink-0" />
                          <span>GitHub Tour</span>
                        </div>
                        <span className="text-[9px] font-bold text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded">Soon</span>
                      </button>
                    </div>
